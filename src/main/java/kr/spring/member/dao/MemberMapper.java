...
public interface MemberMapper {
	...
	@Select("select * from(select * from (select * from gentlepro.msg where sender=#{sender} and s_del=1 order by register_date desc)a left outer join gentlepro.member_detail b on a.receiver=b.id ) join gentlepro.club using(club_num) ")
	public List<MsgVO> selectSentMsg(String sender);
	
	@Select("select * from(select * from (select * from gentlepro.msg where receiver=#{receiver} and r_del=1 order by register_date desc) a left outer join gentlepro.member_detail b on a.sender=b.id) join gentlepro.club using(club_num)")
